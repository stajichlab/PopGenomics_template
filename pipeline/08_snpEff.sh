...
if [ ! -e $SNPEFFOUT/$snpEffConfig ]; then
	...
	java -Xmx$MEM -jar $SNPEFFJAR build -datadir $TOPFOLDER/$SNPEFFOUT/data -c $SNPEFFOUT/$snpEffConfig -gff3 -v $SNPEFFGENOME
fi
...
do
  ...
  do
	...
	java -Xmx$MEM -jar $SNPEFFJAR eff -dataDir $TOPFOLDER/data -v $SNPEFFGENOME $INVCF > $OUTVCF
	...
	$TOPFOLDER/scripts/map_snpEff2domains.py --vcf $OUTVCF --domains $TOPFOLDER/genome/${SNPEFFGENOME}_InterproDomains.txt --output $DOMAINVAR
